...
export default class Fondos extends Component {
  ...
  no_proyecto = ['','AU001, Atención y seguimiento a peticiones recibidas en el despacho del procurador atendidas, up: 01', 'AU002, Casos penales de la región oriente resueltas, up: 02', 'AU003, Delitos cometidos en contra de la libertad de expresión, periodistas y personas defensoras de los derechos humanos investigados, up: 03', 'AU004, Averiguaciones previas del sistema tradicional concluidas, up: 04', 'AU005, Casos penales en materia de delitos electorales resueltos, up: 05', 'AU006, Casos penales determinados, concluidos o resueltos en delitos que atenten contra la mujer y la familia, up: 06', 'AU007, Acuerdos reparatorios generados a través de la aplicación de los mecanismos alternativos de solución de controversias en materia penal en la región poniente, up: 07', 'AU008, Investigación y supervisión de los casos penales con motivo de feminicidio, up: 08', 'AU009, Quejas y denuncias por la posible comisión de conductas indebidas en las que incurran las y los servidores públicos atendidas, up: 09', 'AU010, Intervenciones periciales a autoridades de procuración de justicia para una correcta integración del expediente en casos penales entregados, up: 10', 'AU011, Casos penales del delito de narcomenudeo resueltos, up: 11', 'AU012, Casos penales atendidos por los delitos de secuestro y extorsión, up: 12', 'AU013, Gestión administrativa de recursos humanos,financiera, materiales, de informática, de archivo, de calidad, de aportaciones federales, planeación estratégica realizada, up: 13', 'AU014, Determinación y/o resolución de los casos penales de los delitos de trata de personas, lenocinio y delitos conexos, up: 14', 'AU015, Casos penales de la región poniente resueltas, up: 15', 'AU016, Atenciones ciudadanas en los módulos de atención temprana en la región poniente brindadas, up: 16', 'AU017, Determinación en las carpetas de investigación en las unidades de investigación de la regiones poniente, up: 17', 'AU018, Investigación policial ejecutada, up: 18', 'AU019, Atenciones ciudadanas en los módulos de atención temprana en la región poniente brindadas, up: 20', 'AU020, Acuerdos reparatorios generados a traves de la aplicación de los mecanismos alternativos de solución de controversias en materia penal en la región oriente, up: 21', 'AU021, Determinación en las carpetas de investigación en las unidades de investigación de la regiones oriente, up: 22', 'AU022, Delitos de corrupción resueltos, up: 23', 'AU023, Casos penales determinados, concluidos o resueltos de delitos en materia de desaparición forzada de personas cometidos por particulares, delitos vinculados y de personas no localizadas realizados, up: 24', 'A1D11, Centralizada']
  ...
  render() {
    ...
    today = dd + '/' + mm + '/' + yyyy
    const { tipo_doc, oficio_aut, importe, beneficiario, no_proyecto, numCompro } = this.state
    ...
    return (
      ...
      <form className='form-fondo' onSubmit={this.onSubmit}>
        <Grid className='grid-w'>
            ...
            <Paper className='paper-pm'>
              ...
              <div className='div-f2'>
                <div className='div-con'>
                  ...
                  {(admin === 'ADMIN' || admin === 'MIGUEL' || admin === 'ELOY' || admin === 'TERESA' || admin === 'MARTHA') &&
                    <select
                      className='select-f'
                      value={tipo_doc}
                      onChange={this.onChange}
          ...
          {(admin === 'MIGUEL' || admin === 'TERESA' || admin === 'MARCOS' || admin === 'ELOY' || admin === 'MARTHA') &&
